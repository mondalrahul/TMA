...
$(document).ready( function () {
    ...
    $('#btn-send-enquiry').on('click', function(e){
        ...
        if (user_name.trim().length === 0 || user_email.trim().length === 0 || user_selected_date.trim().length === 0) {
    ...
    $('#btn-send-wire-transfer').on('click', function(e){
        ...
        var form_values = {
            payment_method: ($('select.payment-methods').val() === 'wire-transfer') ? 'Bank Account Transfer' : 'Paypal',
            boat_id: boat_id,
    ...
    $('.product-detail-calendar').datepicker({
            startDate: date, todayHighlight: true
        })
        ...
        .on('changeDate', function (e) {
            ...
            var contentCharter = '<p>Charter Date: <span class="highlight">' + datePicked.getDate() + ' ' + nameMonth + ' ' +datePicked.getFullYear() + '</span></p>';
    ...
    $(document).on('click', '#proceed-to-checkout', function(e) {
        ...
        if (payment_option === 'wire-transfer') {
            if ($('[name="charter-agreement"]').is(':checked') === false) {
                alert('You must check into Charter Agreement before Proceed to checkout.');
                return;
            }
        ...
        var form_values = {
            payment_method: (payment_option === 'wire-transfer') ? 'Bank Account Transfer' : 'Paypal',
            boat_id: boat_id,
            addition_request: addition_request,
            additional_deposite: additional_deposite,
            charter_rate: charter_rate,
    ...
    $(document).on('click', '#btn-proceed-to-checkout', function(e) {
        ...
        setTimeout(function(){document.getElementById("proceed-to-checkout").disabled = false;},8000);
        // Process
        if ($('select.payment-methods').val() === 'paypal') {
        ...
        closeModalDialog('paypalConfirmModal');
         ...
         ajaxSetup();
         ajaxSend('/send-mail/wire-transfer', form_values, 'email-wire-transfer', 'POST');
    })
...
function calBookTimePrice()
{
    ...
    $('[name^=lists-sea-sports-]').each(function(){
        ...
        listSeaSportsWithoutTimeSlots.push({name: elementName, price: elementValue, text: elementText});
    });
    ...
    listSeaSportsWithoutTimeSlots.forEach(function(item, index){
        if ($('[name=' + item.name + ']').is(':checked')) {
            ...
            if (oldValue !== '') {
                storeSeaSportsLists.val(oldValue + ',' + item.name.substring(item.name.lastIndexOf("-") + 1));
            } else {
                storeSeaSportsLists.val(item.name.substring(item.name.lastIndexOf("-") + 1));
            }
        }
    });
    $('.updatepricerefresh').css('opacity' , '0.1');
      if ($('[name="discount_type"]').val() == '0' || $('[name="discount_type"]').val() == '1') {
...
function getNameOfMonth(date) {
    var nameMonthList = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];
    var monthIndex = date.getMonth();
...
function genCheckboxBookTime(data) {
    ...
    data.forEach(function(item, index){
        ...
        var itemHtml = $('<label> <input type="checkbox" id="book-time-item-'+ item.id +'" name="book-time-item" value="' + item.id + '">' + item.timeFrom.toFixed(2) + timeFromType + ' to ' +  item.timeTo.toFixed(2) + timeToType + '</label>');
                ...
                switch (action){
                    case 'login':
                        if (!result.errors) {
                            ...
                            $('#common_error').text('Email or password is wrong. Please check again.');
                        } else {
                    ...
                    case 'valid_date':
                        ...
                        if (result.max_date) {
                            ...
                            alert('You can choose date ' + result.max_date + ' for valid time slots.');
                        } else {
...
function getDatesWithTimeSlot(dateStart, dateEnd) {
    var boatId = $('input[name=boat_id]').val();
    $.get('/boat-book/filterDate', {boatId, dateStart, dateEnd}, function(data) {
        if(data.result === true) {
            window.datesWithTimeSlot = window.datesWithTimeSlot || [];
            window.datesWithTimeSlot = window.datesWithTimeSlot.concat(data.validDateList);
        } 
    }, 'json');
} 

function displayPaypalFee(methodElement) {
    if($(methodElement).val() === 'paypal') {
...
function deleteMyBoats(boatIds) {
    if (!boatIds || !Array.isArray(boatIds) || boatIds.length === 0 || !confirm('Are you sure you want to delete these boats?')) {
        return false;
    }

    $.post('/my-boats/delete', { boatIds : boatIds }, function(data) {

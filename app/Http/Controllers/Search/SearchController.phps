...
class SearchController extends Controller
{
     ...
     */
    public function search($boatType, $dataSearch = [])
    {
        if (empty($boatType)) {
       ...
       echo $boatTypeId; dd(  $marinasIds );
        ...
        if ($country) {
            // Get currency unit
            $countryDetail = $countryLogic->getDetailCountry($country);
     ...
     * @author vduong daiduongptit090@gmail.com
     */
    public function searchProductHome($dataSearchList)
    {
        // Create logic
        $boatLogic = new Logics\BoatLogic();
        ...
        $dataSearch = [
           ...
           'country'       => (isset($dataSearchList['country'])) ? $dataSearchList['country'] : 'Singapore',
        ...
        $countrySearch = (in_array($dataSearch['country'], HomeController::COUNTRY_SEARCH)) ? $dataSearch['country'] : 'Singapore';
    ...
    public function boatDetail($boatId, Request $request)
    {
        ...
        $boatSeaSportsList = $boatSeaSportsLogic->getSeaSportsList();
        ...
        if (Auth::check()) {
            ...
            $userExtend = $boatUserLogic->getUserExtend($user->getAttributes()->user_id);
    ...
    public function validDateBoatDetail(Request $request)
    {
        ...
        if (Auth::check()) {
            $user = Auth::user();
            $userId = $user->getAttributes()->user_id;
            $isCancel=0;
            $boatCharterLogic = new Logics\BoatCharterLogic();
